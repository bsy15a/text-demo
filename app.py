...
核心洞察：
    麦可思数据显示约27%的本科生从事与专业不对口的工作——每三人中就有一人，
    ...
    if step == "collect":
        ...
        with col_b:
            ...
            st.session_state.exp_q4 = st.text_area(
                "💻 你用过哪些软件或工具？用它们做过什么具体的事？有什么成果？",
                value=st.session_state.exp_q4,
                placeholder="比如：\"用 Canva 给社团做了招新海报，最后来了两百多人\"或\"用 Python 帮导师处理了一次实验数据，省了两天手动整理的工夫\"\n\n不用写你只是\"用过\"的软件——写你用工具完成过的事。哪怕只是\"用 Excel 帮班委统计了全班四级成绩\"也算。",
                height=100,
            )
...
1. **关于软件/工具**：从"TA 用工具达成了什么"来判断能力，而不是工具本身。例如"用 Canva 给社团做了招新海报，来了两百多人"比"用过 Canva"有价值得多——前者说明 TA 能用工具产生实际影响力。如果学生只罗列工具名没有具体成果，不要脑补能力。只写"用过XX"而没有说用它做了什么事的，降到最低权重。
